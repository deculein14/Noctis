...
5. NEXT — Rebuild View screen (masked password, re-auth reveal/copy, per-field copy icons, grouped-account sub-list).
6. Remaining popups: custom field name prompt now uses the browser's native `prompt()` (done, simpler than a custom modal); delete confirmation uses native `confirm()` (done). Category picker built as part of step 4.
